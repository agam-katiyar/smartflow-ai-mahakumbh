...
function DensityBar({ density, status }) {
  ...
  return (
    <div className="relative w-full h-2 rounded-full overflow-hidden" style={{ background: 'rgba(42,42,62,0.8)' }}>
      <div
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 glass-card rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-sf-border flex items-center justify-between flex-wrap gap-2">
          <div>
        ...
        {/* SVG Map */}
        <div className="relative p-4" style={{ background: '#0D0D1A', minHeight: 420 }}>
          <svg viewBox="0 0 800 500" className="w-full" style={{ minHeight: 380 }}>
            <defs>
            ...
            {mounted && zones.map((zone) => {
              ...
              return (
                <g key={zone.id}
                  onClick={() => setSelected(zone.id === selected?.id ? null : zone)}
                  ...
                  {zone.status === 'critical' && (
                    <circle cx={svgX} cy={svgY} r={radius * 1.8} fill="none" stroke={color}
                      strokeWidth="1.5" opacity="0.5" className="zone-critical"/>
                  )}
                  {/* Main circle */}
                  ...
                  {/* Zone name */}
                  <text x={svgX} y={svgY + radius + 10} textAnchor="middle"
                    fill="rgba(255,255,255,0.7)" fontSize="7" fontFamily="Outfit">
            ...
            <g transform="translate(20, 445)">
              <text fill="rgba(255,255,255,0.35)" fontSize="8" fontFamily="Outfit">
                🔵 Rivers  🟠 Shuttle Routes  ● Zone Density
              </text>
            </g>
          </svg>
          ...
          {/* Hover tooltip */}
          {hoveredZone && (
            <div className="absolute top-4 right-4 custom-tooltip z-10" style={{ minWidth: 180 }}>
        ...
        <div className="p-4 border-t border-sf-border">
          <p className="text-xs text-gray-400 mb-3 font-medium uppercase tracking-wide">Active Shuttle Lines</p>
            ...
            .map(zone => (
              <div
                ...
                className={`p-3 cursor-pointer transition-all ${
                  selected?.id === zone.id ? 'bg-sf-card' : 'hover:bg-sf-card/50'
                }`}
                style={{ borderBottom: '1px solid rgba(42,42,62,0.5)' }}
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="status-dot" style={{ background: DENSITY_COLORS[zone.status]?.bg, width: 8, height: 8, borderRadius: '50%', display: 'inline-block' }}></span>
                    <span className="text-sm font-medium text-white" style={{ maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{zone.name}</span>
                  </div>
                ...
                <div className="flex justify-between mt-1 text-xs text-gray-500">
                  ...
                  <span>{Math.round(zone.capacity * zone.density / 100).toLocaleString()} / {zone.capacity.toLocaleString()}</span>
                </div>
              </div>
            ))}
        </div>
